...
contract Election1 {
    ...
    uint public candidatesCount1;

    // Store which candidate each voter voted for
    mapping(address => uint) public voterToCandidate1;
    ...
    function addCandidate (string memory _name1) private {
        candidatesCount1 ++;
        candidates1[candidatesCount1] = Candidate1(candidatesCount1, _name1, 0);
    }

    function vote1 (uint _candidateId1) public {
        ...
        require(_candidateId1 > 0 && _candidateId1 <= candidatesCount1);

        // record that voter has voted
        voters1[msg.sender] = true;

        // store the vote
        voterToCandidate1[msg.sender] = _candidateId1;

        // update candidate vote Count
        candidates1[_candidateId1].voteCount1 ++;

        // trigger voted event
        emit votedEvent1(_candidateId1);
    }

    function revokeVote1 () public {
        // require that they have voted before
        require(voters1[msg.sender]);

        // update candidate vote Count
        candidates1[voterToCandidate1[msg.sender]].voteCount1 --;

        // record that voter has voted
        voters1[msg.sender] = false;

        // trigger voted event
        emit revocatedEvent1(voterToCandidate1[msg.sender]);
    }
}
