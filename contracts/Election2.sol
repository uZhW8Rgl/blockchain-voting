...
contract Election2 {
    ...
    uint public candidatesCount2;

    // Store which candidate each voter voted for
    mapping(address => uint) public voterToCandidate2;
    ...
    function addCandidate (string memory _name2) private {
        candidatesCount2 ++;
        candidates2[candidatesCount2] = Candidate2(candidatesCount2, _name2, 0);
    }

    function vote2 (uint _candidateId2) public {
        ...
        require(_candidateId2 > 0 && _candidateId2 <= candidatesCount2);

        // record that voter has voted
        voters2[msg.sender] = true;

        // store the vote
        voterToCandidate2[msg.sender] = _candidateId2;

        // update candidate vote Count
        candidates2[_candidateId2].voteCount2 ++;

        // trigger voted event
        emit votedEvent2(_candidateId2);
    }

    function revokeVote2 () public {
        // require that they have voted before
        require(voters2[msg.sender]);

        // update candidate vote Count
        candidates2[voterToCandidate2[msg.sender]].voteCount2 --;

        // record that voter has voted
        voters2[msg.sender] = false;

        // trigger voted event
        emit revocatedEvent2(voterToCandidate2[msg.sender]);
    }
}
